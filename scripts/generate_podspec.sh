...
# Generate the Ruby .podspec.json file with updated name and dependencies (source of truth for Ruby output below)
...
jq \
  ...
  ' \
  "${ORIGINAL_PODSPEC_FILE_PATH}" > "${PODSPEC_JSON}"
echo "✅ Wrote ${PODSPEC_JSON}" >&2

# Generate the Ruby .podspec file
PODSPEC_RUBY="${POD_NAME}.podspec"
ver=$(jq -r '.version | @json' "${PODSPEC_JSON}")
summary=$(jq -r '.summary | @json' "${PODSPEC_JSON}")
description=$(jq -r '.description | @json' "${PODSPEC_JSON}")
homepage=$(jq -r '(.homepage // "http://www.is.com/") | @json' "${PODSPEC_JSON}")
lic_type=$(jq -r '(.license.type // "Commercial") | @json' "${PODSPEC_JSON}")
lic_text=$(jq -r '(.license.text // "") | @json' "${PODSPEC_JSON}")
platform_ios=$(jq -r '((.platforms.ios) // "14.0") | @json' "${PODSPEC_JSON}")
source_files=$(jq -r '.source_files | @json' "${PODSPEC_JSON}")
swift_versions=$(jq -r '(.swift_versions // .swift_version // "5.0") | @json' "${PODSPEC_JSON}")
...

{
  ...
  if jq -e 'has("public_header_files") and (.public_header_files | type) != "null"' "${PODSPEC_JSON}" >/dev/null 2>&1; then
    phf=$(jq -r '.public_header_files | @json' "${PODSPEC_JSON}")
    echo "  s.public_header_files = ${phf}"
  fi
  if jq -e 'has("preserve_paths") and (.preserve_paths | type) != "null"' "${PODSPEC_JSON}" >/dev/null 2>&1; then
    pp=$(jq -r '.preserve_paths | @json' "${PODSPEC_JSON}")
    echo "  s.preserve_paths = ${pp}"
  fi
  if jq -e 'has("vendored_frameworks") and (.vendored_frameworks | type) != "null"' "${PODSPEC_JSON}" >/dev/null 2>&1; then
    vf=$(jq -r '.vendored_frameworks | @json' "${PODSPEC_JSON}")
    echo "  s.vendored_frameworks = ${vf}"
  fi

  if jq -e '(.pod_target_xcconfig // {}) | length > 0' "${PODSPEC_JSON}" >/dev/null 2>&1; then
    echo
